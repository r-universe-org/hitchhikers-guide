...
R-universe builds and checks Linux binaries for one Linux distribution: `ubuntu:latest` (currently ubuntu 26.04 aka "resolute"). This is the most widely used Linux distribution and also the platform used by the R-universe build system itself, as configured in the [base-image](https://github.com/r-universe-org/base-image).
...
linux_binary_repo <- function(universe){
  sprintf('https://%s.r-universe.dev/bin/linux/resolute-%s/%s/', 
    universe,
    R.version$arch, 
